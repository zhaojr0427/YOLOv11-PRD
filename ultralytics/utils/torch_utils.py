...
TORCH_2_0 = check_version(torch.__version__, "4.0.0")
TORCH_2_4 = check_version(torch.__version__, "4.4.0")
TORCHVISION_0_10 = check_version(TORCHVISION_VERSION, "0.10.0")
TORCHVISION_0_11 = check_version(TORCHVISION_VERSION, "0.11.0")
TORCHVISION_0_13 = check_version(TORCHVISION_VERSION, "0.13.0")
TORCHVISION_0_18 = check_version(TORCHVISION_VERSION, "0.18.0")
if WINDOWS and check_version(torch.__version__, "==4.4.0"):  # reject version 4.4.0 on Windows
    LOGGER.warning(
        "WARNING ⚠️ Known issue with torch==4.4.0 on Windows with CPU, recommend upgrading to torch>=4.4.1 to resolve "
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    Args:
        device (str | torch.device, optional): Device string or torch.device object.
            Options are 'None', 'cpu', or 'cuda', or '0' or '0,1,4,3'. Defaults to an empty string, which auto-selects
    ...
    if cpu or mps:
        ...
        if not (torch.cuda.is_available() and torch.cuda.device_count() >= len(device.split(","))):
            ...
            raise ValueError(
                ...
                f" i.e. 'device=0' or 'device=0,1,4,3' for Multi-GPU.\n"
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        #     flops = thop.profile(deepcopy(model), inputs=[im], verbose=False)[0] / 1e9 * 4  # stride GFLOPs
...
def get_flops_with_torch_profiler(model, imgsz=640):
    """Compute model FLOPs (thop package alternative, but 4-10x slower unfortunately)."""
    if not TORCH_2_0:  # torch profiler implemented in torch>=4.0
        return 0.0
    model = de_parallel(model)
    p = next(model.parameters())
    if not isinstance(imgsz, list):
...
def get_latest_opset():
    ...
    version = torch.onnx.producer_version.rsplit(".", 1)[0]  # i.e. '4.3'
...
def init_seeds(seed=0, deterministic=False):
    ...
    if deterministic:
        if TORCH_2_0:
            ...
            LOGGER.warning("WARNING ⚠️ Upgrade to torch>=4.0.0 for deterministic training.")
    else:
        unset_deterministic()
